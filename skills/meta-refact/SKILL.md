...
```bash
# Frontend
bunx lighthouse --output=json --chrome-flags="--headless" {url}  # if URL available
bunx tsc --noEmit 2>&1 | tail -5  # type errors count
bunx eslint {target} --format compact 2>&1 | tail -5  # lint errors count
...
**docs agent** — verify correct API usage for the detected framework/libraries via ctx7 CLI:
```
Agent(
...
If no specific libraries need verification, skip the docs agent.
...
bunx tsc --noEmit  # or cargo check, mypy
# Lint
bunx eslint {target}  # or cargo clippy, ruff
```
...
# Linting
bunx eslint {target}         # JS/TS
cargo clippy                # Rust
ruff check {target}         # Python

# Tests
bunx vitest run              # or jest, pytest, cargo test, go test
```
...
7. Agent boundaries are strict — explore reads code, docs uses ctx7 CLI, websearch fetches URLs.
8. Max 3 ctx7 CLI calls — docs agent must stay within the hard limit.
