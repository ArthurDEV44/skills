...
- [Step 3b: docs agent Prompt Template (ctx7 CLI)](#step-3b-docs-agent-prompt-template-ctx7-cli)
...
All agents are spawned using the `Agent` tool with `subagent_type`. meta-refact is a pipeline, not a long-lived team.
...
## Step 3b: docs agent Prompt Template (ctx7 CLI)
...
```
Look up official documentation using the ctx7 CLI to verify correct API usage and find optimization patterns for the libraries used in this refactoring.
...
[ctx7 library IDs and query topics used]
```
...
If no specific libraries need doc verification, omit the docs agent call.
...
1. Official docs (docs agent) — highest authority for API correctness
...
The orchestrator NEVER duplicates agent work. It does not explore the broader codebase architecture (that's agent-explore), search the web (that's agent-websearch), or run ctx7 CLI (that's the docs agent). It reads the target files, analyzes them directly, orchestrates agents, and applies fixes.
